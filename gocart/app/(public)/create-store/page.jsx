...
try {
    ...
    if (['approved', 'rejected', 'pending'].includes(data.status)) {
        ...
        switch (data.status) {
            ...
            case "rejected":
             ...
             case "pending":
                setMessage(
                    "Your store request is pending, contact the admin for more details"
                );
                break;
    ...
    const onSubmitHandler = async (e) => {
        ...
        try {
            const token = await getToken(); // Get auth token (e.g. from Clerk or custom auth)
            ...
            formData.append("image", storeInfo.image); // Assumes image is a File or Blob
